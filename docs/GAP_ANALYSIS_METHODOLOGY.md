...
**Application**: Change `{TASK_NUMBER}` throughout this document to target any task suite (e.g., 001, 002, 050, 051).
...
**Expected Output**:
```
docs/tasks/refined-tasks/Epic NN/task-{TASK_NUMBER}-parent-task-name.md
docs/tasks/refined-tasks/Epic NN/task-{TASK_NUMBER}a-subtask-name.md
docs/tasks/refined-tasks/Epic NN/task-{TASK_NUMBER}b-subtask-name.md
...
```
...
wc -l docs/tasks/refined-tasks/Epic\ NN/task-{TASK_NUMBER}*.md

# Find Implementation Prompt section line number (contains complete production code)
grep -n "## Implementation Prompt" docs/tasks/refined-tasks/Epic\ NN/task-{TASK_NUMBER}a*.md

# Find Testing Requirements section line number (contains complete test code)
grep -n "## Testing Requirements" docs/tasks/refined-tasks/Epic\ NN/task-{TASK_NUMBER}a*.md

# Find Acceptance Criteria section (for feature checklist)
grep -n "## Acceptance Criteria" docs/tasks/refined-tasks/Epic\ NN/task-{TASK_NUMBER}a*.md
```
...
Read(file_path="docs/tasks/refined-tasks/Epic NN/task-{TASK_NUMBER}a-*.md",
...
**File Location**: `docs/implementation-plans/task-{TASK_NUMBER}a-gap-analysis.md`

**Document Structure**:

```markdown
# Task {TASK_NUMBER}a Gap Analysis
...
```json
[
  {"content": "Task {TASK_NUMBER}a Phase N: [Component] (INCOMPLETE → COMPLETE)", "status": "in_progress", "activeForm": "Completing [Component]"},
  {"content": "Task {TASK_NUMBER}a Phase N+1: [Next]", "status": "pending", "activeForm": "Next phase"}
]
```
...
```bash
git add -A
git commit -m "feat(task-{TASK_NUMBER}a): complete [Component] implementation

Phase N of task-{TASK_NUMBER}a:
...
git push origin feature/task-{TASK_NUMBER}-[branch-name]
```

**Step 7: Update Gap Analysis Document**
```bash
# Mark phase complete with evidence
Edit(file_path="docs/implementation-plans/task-{TASK_NUMBER}a-gap-analysis.md",
...
```json
{"content": "✅ Task {TASK_NUMBER}a Phase N: [Component] COMPLETE", "status": "completed", "activeForm": "Verified complete with tests"}
```
...
Read(file_path="docs/implementation-plans/task-{TASK_NUMBER}a-gap-analysis.md")
...
### Quick Start: Gap Analysis for Task {TASK_NUMBER}
...
```bash
SPEC_FILE="docs/tasks/refined-tasks/Epic NN/task-{TASK_NUMBER}a-*.md"
wc -l "$SPEC_FILE"
...
Copy this template and replace `{TASK_NUMBER}`:

```markdown
# Task {TASK_NUMBER} Gap Analysis and Completion - Execution Instructions

## Your Mission
Verify and complete task-{TASK_NUMBER} by implementing ALL components from the specification and PROVING they work with passing tests.
...
```bash
# Find spec file
SPEC_FILE=$(find docs/tasks/refined-tasks -name "task-{TASK_NUMBER}a-*.md")
...
Create: `docs/implementation-plans/task-{TASK_NUMBER}a-gap-analysis.md`
...
```bash
git add -A
git commit -m "feat(task-{TASK_NUMBER}a): complete [Component]
...
**Reusability**: Replace `{TASK_NUMBER}` to target any task suite.
